...
def get_oper_category_count(oper_name,big_or_sub):
    #---- init 2rd dict
...
def get_oper_category(oper_name,big_or_sub):
...
for i in range(len(oper_list)):
    ...
    get_oper_category(oper_list[i],big_or_sub)
    get_oper_category_count(oper_list[i],big_or_sub)
...
'''
# display === way 1, p.wedge
'''
p = figure(x_range=(-1.5, 1.7), y_range=(-1.5, 1.5), title=oper_list[0]) 
oper_total_num = sum(category_dict_num[oper_list[0]].values())
print('oper_total_num = ' + str(oper_total_num))

previous_end_angel = 0
#colors = itertools.cycle(palette) 
for idx, cate_pair in enumerate(sorted(category_dict_num[oper_list[0]].items(), key=lambda x: x[1])):
    print('idx = ' + str(idx) + ' cate_name=' + str(cate_pair[0]) + ' cate_num= ' + str(cate_pair[1]))
    angle_start = 0 if idx == 0 else previous_end_angel
    angle_end   = previous_end_angel + cate_pair[1]/oper_total_num
    previous_end_angel = angle_end
    p.wedge(x=0 , y=0, radius=1, start_angle=angle_start*2*pi, end_angle=angle_end*2*pi, 
                                color=Category20c[20][idx%20],legend=cate_pair[0]) 
    p.text(0, radii[:-1], cate_pair[1],
...
for op_idx in range(len(oper_list)):
    ...
    if len(this_category) < 20:
        data['color'] = Category20c[len(this_category)] 
    else:
